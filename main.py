...
def main():
    ...
    parser.add_argument(
        ...
        choices=["ml-20m", "netflix", "lfm2b", "ml1m"],
    )
    parser.add_argument(
        "--processed_dir", type=str, default="../data/lfm2b/", help="dataset directory"
    )
    ...
    parser.add_argument(
        "--checkpoint_freq", type=int, default=1, help="checkpoint save frequency"
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    parser.add_argument(
        "--start_step", dest="start_step", type=int, default=0, help="start step"
    )
    parser.add_argument(
        "--total_steps",
        dest="total_steps",
    ...
    parser.add_argument(
        "--valid_batch_size", type=int, default=100, help="batch size in validation"
    )
    parser.add_argument(
        "--test_batch_size", type=int, default=100, help="batch size in test"
    )
    parser.add_argument("--print_freq", type=int, default=1, help="log print frequency")
    parser.add_argument(
        "--upper_train", type=int, default=-1, help="max of train images(for debug)"
    )
    parser.add_argument(
        "--upper_valid", type=int, default=-1, help="max of valid images(for debug)"
    )
    parser.add_argument(
        "--upper_test", type=int, default=-1, help="max of test images(for debug)"
    )
    parser.add_argument(
        "--total_anneal_steps",
        type=int,
    ...
    parser.add_argument(
        "--beta", type=float, default=1.0, help="largest annealing parameter"
    )
    parser.add_argument(
        "--gamma", type=float, default=5.0, help="largest annealing parameter"
    )
    parser.add_argument(
        "--tau", type=float, default=0.5, help="largest annealing parameter"
    )
    parser.add_argument(
        "--dropout_p", dest="dropout_p", type=float, default=0.5, help="dropout rate"
    )
    parser.add_argument("--gpu", type=int, default=0, help="GPU id")
    parser.add_argument(
        "--target", type=str, default="country", help="Activate Bias Method"
    )
    parser.add_argument(
        "-j",
        "--workers",
    ...
    item_mapper = pd.read_csv(root + "items.csv", header=[0])
    user_mapper = pd.read_csv(root + "users.csv", header=0)
    ...
    if args.target == "country" and args.dataset_name == "lfm2b":
        ...
        labels = (
            labels.replace(labels_others.country.values, "OTHERS")
            .filter(["country"])
            .drop_duplicates()
        )
        ...
        user_mapper["country"] = user_mapper.country.replace(
            labels_others.country.values, "OTHERS"
        )
    ...
    #################################
    unique_sid = item_mapper.sid.unique()
    # unique_sid = item_mapper.columns
    ...
    train_loader = torch.utils.data.DataLoader(
        dt, batch_size=args.train_batch_size, shuffle=False, **kwargs
    )
    ...
    if args.cmd == "train":
        model = MultiVAE(
            ...
            weight_decay=0.0,
            cuda2=cuda,
            q_dims=[args.n_items, 2000, 200],
            p_dims=[200, 2000, args.n_items],
        ...
        checkpoint = torch.load(
            "checkpoint/{}_vae_{}_{}_{}_{}.pth".format(
                args.dataset_name, args.target, args.beta, args.gamma, args.tau
            )
        )
